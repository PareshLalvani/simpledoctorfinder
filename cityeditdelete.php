...
{
	var http = '<?php if (isset($_SESSION['state_code'])){unset($_SESSION['state_code']);}?>';
	var http = new XMLHttpRequest();
	http.open("GET","includes/getstatecities.php?state_code="+selstate, true);
	http.send();
...
<article class="module width_half">
		<header><h3 class="tabs_involved">CITY Table Maintenance</h3></header>
	    <form method="POST">
					<fieldset>
                   		<label>STATE</label>
						...
						<?php
                        	 ...
                        	 {
                      	?>
                         	 <option value="<?php echo $res1->STATE_CODE; ?>"<?php if(isset($_SESSION['state_code'])){if ($_SESSION['state_code'] == $res1->STATE_CODE){echo "selected";}}
							?>><?php echo $res1->STATE_NAME; ?></option>
                       	<?php
                       		 }
                       	?>
                   		 </select>
...
jQuery("#target-content").load("includes/getstatecities.php?page=1");
    ...
    e.preventDefault();
        ...
        var pageNum = this.id;
        jQuery("#target-content").load("includes/getstatecities.php?page=" + pageNum);
    });
    });
</script>
</html>